...
import time
import math
from data_utils_dendritic import *
...
class SparseDendriticBranch(torch.nn.Module):
    """
    Sparse local dendritic transform:
    presynaptic input -> sparse masked branches -> nonlinearity -> fixed aggregation per soma.
    """
    def __init__(self, in_dim, out_dim, branches_per_soma=4, dense = False,  branch_sparsity=0.2, activation=torch.tanh, bias=True):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.branches_per_soma = branches_per_soma
        self.total_branches = out_dim * branches_per_soma
        self.activation = activation
        self.dense = dense

        self.weight = torch.nn.Parameter(torch.empty(self.total_branches, in_dim))
        self.bias = torch.nn.Parameter(torch.zeros(self.total_branches)) if bias else None

        if self.dense:
            torch.nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))     
            keep = self.in_dim
        else:
            keep = max(1, int(round(branch_sparsity * in_dim)))
            mask = torch.zeros(self.total_branches, in_dim)
            for b in range(self.total_branches):
                idx = torch.randperm(in_dim)[:keep]
                mask[b, idx] = 1.0
    ...
    def forward(self, x):
        if self.dense:
            ...
            z = F.linear(x, masked_weight, self.bias)

        #z = self.activation(z)
        z = my_relu(z)
        z = z.view(x.size(0), self.out_dim, self.branches_per_soma).sum(dim=2)
        return z / self.branches_per_soma


class Dendritic_MLP(torch.nn.Module):
    """
    EP-compatible dendritic MLP.
    Hidden layers receive:
      - basal sparse nonlinear dendritic drive from layer l-1
      - apical sparse nonlinear dendritic drive from layer l+1
    The output layer remains standard EP.
    """
    def __init__(self, archi, activation=torch.tanh, basal_branches=4, apical_dense = False, apical_branches=2,
                 branch_sparsity=0.2, apical_scale=1.0):
        super().__init__()

        if len(archi) < 3:
            raise ValueError("Dendritic_MLP requires at least input, one hidden, and output layers.")

        self.activation = activation
        self.archi = archi
        self.softmax = False
        self.nc = self.archi[-1]
        self.basal_branches = basal_branches
        self.dense = apical_dense
        self.apical_branches = apical_branches
        self.branch_sparsity = branch_sparsity
        self.apical_scale = apical_scale
    

        self.synapses = torch.nn.ModuleList()
        for idx in range(len(archi) - 1):
            self.synapses.append(
                SparseDendriticBranch(
                    archi[idx],
                    archi[idx + 1],
                    branches_per_soma=basal_branches,
                    branch_sparsity=branch_sparsity,
                    dense=False,
        ...
        for idx in range(1, len(archi) - 1):
            self.B_syn.append(
                SparseDendriticBranch(
                    archi[idx + 1],
                    archi[idx],
                    branches_per_soma=apical_branches,
                    branch_sparsity=branch_sparsity,
                    dense=self.dense, 
                    activation=activation,
                    bias=False
    ...
    def Phi(self, x, y, neurons, beta, criterion):
        ...
        for idx in range(len(neurons) - 1):
            basal = self.synapses[idx](layers[idx])
            apical = self.B_syn[idx](layers[idx + 2])
            phi = torch.sum((basal + self.apical_scale * apical) * layers[idx + 1], dim=1).squeeze()
            phis.append(phi)
    ...
    def forward(self, x, y, neurons, T, beta=0.0, criterion=torch.nn.MSELoss(reduction='none'), check_thm=False):
        ...
        for t in range(T):
            ...
            for idx in range(len(neurons) - 1):
                init_grad = torch.ones(mbs, dtype=torch.float, device=device, requires_grad=check_thm)
    ...
    def compute_syn_grads(self, x, y, neurons_1, neurons_2, betas, criterion, check_thm=False, neurons_3=None):
        ...
        if neurons_3 is None:
            ...
            # Symmetric third phase: use +beta and -beta fixed points directly.
            phis_2 = self.Phi(x, y, neurons_3, beta_2, criterion)
...
def train(model, optimizer, train_loader, test_loader, T1, T2, betas, device, epochs, criterion, alg='EP', 
    ...
    if save:
        np.save(path + '/train_acc.npy', np.array(train_acc))
        np.save(path + '/test_acc.npy', np.array(test_acc))
        save_dic = {'model_state_dict': model.state_dict(), 'opt': optimizer.state_dict(),
                    'train_acc': train_acc, 'test_acc': test_acc, 
