...
import math
import sys
from model_utils_dendritic import *
...
parser.add_argument('--basal-branches', type=int, default=4, help='number of basal dendritic subunits per soma for DMLP')
parser.add_argument('--apical-branches', type=int, default=2, help='number of apical dendritic subunits per soma for DMLP')
parser.add_argument('--branch-sparsity', type=float, default=0.2, help='fraction of presynaptic units connected to each dendritic branch for DMLP')
parser.add_argument('--apical-scale', type=float, default=1.0, help='scaling factor for apical feedback in DMLP')
parser.add_argument('--apical-dense', default = False, action = 'store_true', help='enabling dense feedback')
...
if args.task=='MNIST':
    ...
    transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                torchvision.transforms.Normalize((0.2860,), (0.3530,)),  # Fashion-MNIST mean and std
    ...
    train_loader = torch.utils.data.DataLoader(fmnist_dset_train, batch_size=mbs, shuffle=True, num_workers=0)
    ...
    test_loader = torch.utils.data.DataLoader(fmnist_dset_test, batch_size=200, shuffle=False, num_workers=0)

elif args.task=='KMNIST':

    transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                #torchvision.transforms.Normalize((0.5,), (0.5,)),  # keep same as your current setup for fair comparison
                                                torchvision.transforms.Normalize((0.1918,), (0.3483,)),
                                                torchvision.transforms.Lambda(lambda x: x.view(-1))])

    kmnist_dset_train = torchvision.datasets.KMNIST('.', train=True, download=False, transform=transform)
    train_loader = torch.utils.data.DataLoader(kmnist_dset_train, batch_size=mbs, shuffle=True, num_workers=0)

    kmnist_dset_test  = torchvision.datasets.KMNIST('.', train=False, download=False, transform=transform)
    test_loader = torch.utils.data.DataLoader(kmnist_dset_test, batch_size=200, shuffle=False, num_workers=0)
...
print("apical dense:", args.apical_dense)
...
if args.load_path=='':
    ...
    if args.model=='MLP':
        ...
        model = Dendritic_MLP(
            ...
            basal_branches=args.basal_branches,
            apical_branches=args.apical_branches,
            branch_sparsity=args.branch_sparsity,
            apical_dense = args.apical_dense,
            apical_scale=args.apical_scale
        )
